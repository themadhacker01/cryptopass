...
contract CryptoPass {
    ...
    string[] public usedUname;                  // start adding address in arra
    ...
    function setValue(address _pubKey, string _userName) public {        
        ...
        if (flag==0){            
            ...
            keyUserName[_pubKey] = _userName;       //Flag = 0 means no error            
        }
    }
    ...
    function stringsEqual(string storage _a, string memory _b) internal returns (bool) {
        ...
        for (uint i = 0; i < a.length; i++)
            if (a[i] != b[i]) {
                return false;       // Byte values of string are the same
            }
        return true;
    }
}